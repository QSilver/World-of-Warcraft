	...
	if Lib then
		...
		local function _StartupThread()
			...
			local function punchStartupTime(desc)
				...
				local t2b = timems()
				local time = t2b-t2
				tinsert(Lib.StartupTimes,("%d ms - %s (%s frames, %d ms avg)"):format(time,desc,framecount,time/framecount))
			...
			local function resumeStartupTime()
				t2 = t2 + timems() - pausetime
			end

			local yield = function(s)
				framecount=framecount+1
				return yield(s)
			end
			...
			yield("maxspeeds") framecount=framecount+1
			...
			repeat yield("startup_suspended") until ZGV.db.profile.pathfinding ---------==============  H A L T   H E R E  ================-----
			...
			do -- INITIALIZE INNS
				for z,zone in pairs(Lib.data.basenodes.inns) do
					...
					pauseStartupTime()  yield("inns "..z)  resumeStartupTime()
				end
			end
			...
			do -- INITIALIZE BORDERS
				for c,cont in pairs(Lib.data.basenodes.borders) do  -- yes, c is useless
					for d,data in ipairs(cont) do
						...
						if d%5==0 then pauseStartupTime()  yield("borders "..c)  resumeStartupTime() end
					end
				end
			...
			do -- INITIALIZE TRAVEL
				for d,data in ipairs(Lib.data.basenodes.travel) do
					SmartAddNode(data)
					if d%2==0 then  pauseStartupTime()  yield("travel")  resumeStartupTime()  end
				end
				Lib.data.basenodes.travel = nil
				--local t2b=timems()  print("LibRover initialization: travel ",t2b-t2)  t2=t2b
				--@~320ms
			end
			...
			do -- INITIALIZE EXPLICIT FLOORING
				...
				for id,zonedata in pairs(Lib.data.basenodes.MapsWithExplicitFloors) do
					for n,nodedata in ipairs(zonedata) do
						local node1,node2=SmartAddNode(nodedata)
						if node1 then node1.flooring=true end
						if node2 then node2.flooring=true end
						i=i+1  if i%5==0 then  pauseStartupTime()  yield("flooring")  resumeStartupTime()  end
					end
			...
			do -- INITIALIZE INDOOR AREAS
				...
				for id,data in pairs(Lib.data.basenodes.indoorzones) do
					for n,node in ipairs(data) do
						SmartAddNode(node)
						i=i+1  if i%5==0 then  pauseStartupTime()  yield("indoors")  resumeStartupTime()  end
					end
			...
			do -- INITIALIZE PORTKEYS
				for i,item in ipairs(Lib.data.portkeys) do repeat
					...
					if i%5==0 then pauseStartupTime()  yield("portkeys")  resumeStartupTime() end
		...
		function Lib:StartupStep()
			...
			while thisframe<STARTUP_INTENSITY do
				...
				if ret=="startup_suspended" then return end
		...
		function Lib:QueueFindPath(am,af,ax,ay,bm,bf,bx,by, handler, extradata, force_new, quiet)
			ZGV:Debug("Librover: Adding new task for findpath")
			...
			if not self.trytofindpathexecute then
				ZGV:Debug("Librover: Timer inactive, starting")
				self.trytofindpath = self:ScheduleRepeatingTimer("TryToFindPath", 0.1)
			end
		end

		function Lib:TryToFindPath()
			if #Lib.delayeddata == 0 then
				ZGV:Debug("Librover: No more jobs in queue, stopping timer")
				self:CancelTimer(self.trytofindpath)
				self.trytofindpath = nil
				--self:Abort()
				return
			end
			...
			if not Lib.calculating then
				...
				ZGV:Debug("Librover: Started next job at FindPath")
			end
		end
		...
		function Lib:FindPath(am,af,ax,ay,bm,bf,bx,by, handler, extradata, force_new, quiet)
			...
			self:Debug("IsDestinationImpossible? %s,%s,%s",tostring(not not is_impossible),tostring(code),tostring(reason or ""))
			if is_impossible then
				self:Debug("&_POP FindPath: failing because %s.",tostring(code))
			...
			if not self.ready then
				...
				self:Debug("&_POP FindPath: Saving for after startup")
				return handler and handler("PROGRESS")
			end


			if not am or not af or not bm or not bf or am==0 or bm==0 then  self:ReportFail("Cannot determine start or end location.")  self:Debug("&_POP FindPath failed: no start or end map")  return   end
			...
			if not start_is_new and not end_is_new then  self:UpdateNow()  self:Debug("&_POP FindPath: UpdateNow")  return end
		...
		function Lib:InitializePath()
			...
			self:Debug("&_PUSH InitializePath")
			...
			self:Debug("&LRadv InitializePath: cleared old nodes, %d left",#all)
			...
			self:Debug("&LRadv InitializePath: cleared linkage")
			...
			if end_is_new then
				...
				AddNode(self.copyendnode)
				 ...
				 self:Debug("&LRadv Initialized end node #".. self.endnode.num .. " with copy #".. self.copyendnode.num.." "..tostring(self.copyendnode))
			end
			...
			if start_is_new or self.force_next then
				...
				self:Debug("&LRadv Initialized start node #".. self.startnode.num)
			end
			...
			if self.extradata and self.extradata.multiple_ends then
				...
				if limit<0 then self:Debug("multiple_ends limit reached.") end
			end
			...
			self:Debug("&LRadv InitializePath: cleared temp fields")
			...
			self:Debug("&_POP InitializePath: done")
		...
		function Lib:ReportPath(endnode)
			...
			-- TAXI DISPLAY PREPARATION: find the final flight and stamp it onto all flights in a sequence.
				...
				local final_node
				for i=#results-1,2,-1 do repeat --find the flights that are not final
			...
			--=========== HAIRPIN OPTIMIZATION
				...
				and
					(
						( -- standing on the point
							getdist(sn,n1)<(tonumber(n1.radius) or (IsFlying("player") and 50 or 10))
						...
						( -- standing next to the point, acute angle, same floor
							...
							and save_reason("acute angle "..AngleBetween(sn,n1,n2) , true)
						)
							--[[
								or
								(
				...
				do
					Lib:Debug("|cffff8800skipped hairpin node|r (because %s): %s",reason,n1:tostring())
		...
		function Lib:Abort()
			ZGV:Debug("Librover: clearing queue")
			Lib.delayeddata = {}
			self:CancelTimer(self.trytofindpath)
			self.trytofindpath = nil
