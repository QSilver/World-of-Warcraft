...
data.MapIDsByNameR = {}
for k,v in pairs(data.MapIDsByName) do data.MapIDsByNameR[v]=k end
...
data.basenodes.MapsWithExplicitFloors = {
		...
		-- Cataclysm areas
			...
			["Gilneas"] = {
				...
				"678/0 28.8,51.2 -x- 678/2 69.9,63.4 {template:building}", -- Greymane Manor
				"678/2 60.3,46.2 -x- 678/3 56.1,46.5", -- Greymane Manor2
				"679/0 76.6,31.1 -x- 679/1 28.5,70.0 {template:mine}", -- Emberstone Mine
				"679/0 28.8,51.2 -x- 679/2 69.9,63.4 {template:building}", -- Greymane Manor
				"679/2 60.3,46.2 -x- 679/3 56.1,46.5", -- Greymane Manor2
			},
		...
		-- Draenor
			["Frostfire Ridge"] = {
				...
				"Frostfire Ridge/0 29.7,41.5 -x- Frostfire Ridge/4 71.3,89.6 {template:mine}", --Sootstained Mines
				...
				"Frostfire Ridge/0 63.3,65.7 -x- Frostfire Ridge/6 24.6,58.6 {template:building}", --The Hunter's Hall
			},
			["Frostwall"] = {
				"Frostwall/0 63.4,72.8 -x- Frostwall/26 65.4,55.0 {template:mine}", --Frostwall Mine, no upgrade
				"Frostwall/0 63.4,72.8 -x- Frostwall/27 65.4,55.0 {template:mine}", --Frostwall Mine, upgraded
			},
			["Gorgrond"] = {
				"Gorgrond/0 47.7,94.4 -x- Gorgrond/16 39.3,31.4 {template:cave}", --Moira's Bastion
				"Gorgrond/16 58.7,83.1 -x- Gorgrond/17 59.4,84.5", --The Armory
				"Gorgrond/0 43.7,48.3 -x- Gorgrond/18 62.0,80.8 {template:cave}", --Fissure of Fury
				"Gorgrond/18 68.3,51.5 -x- Gorgrond/19 64.7,60.8", --Heart of Fury
			},
			["Lunarfall"] = {
				"Lunarfall/0 66.8,42.6 -x- Lunarfall/24 67.9,63.6 {template:mine}", --Lunarfall Excavation
			},
			["Nagrand D"] = {
				"Nagrand D/0 56.4,61.8 -x- Nagrand D/12 94.7,34.6 {template:cave}", --The Underpale
				"Nagrand D/0 67.0,49.6 -x- Nagrand D/11 11.8,77.1 {template:cave}", --Vault of the Titan
				"Nagrand D/0 47.2,64.4 -x- Nagrand D/12  9.1,60.8 {template:cave}", --The Underpale
				"Nagrand D/0 47.1,62.1 -to- Nagrand D/0 47.0,62.0 {template:exit}", --The Underpale  -- TODO: won't work. Same map!
			},
			["Shadowmoon Valley D"] = {
			...
			["Talador"] = {
				...
				"Talador/0 58.6,65.1 -x- Talador/13 30.1,33.1 {template:cave}", --Tomb of Lights
			},
...
data.basenodes.borders = {
	...
	[962] = {
		...
		--"Frostfire Ridge/0 44.15,68.52 -x- Frostwall/0 16.21,73.99 {template:custom} {atob:Enter Garrison} {btoa:Leave Garrison}", -- 941 is initial Frostfire map
		--"Frostfire Ridge/0 49.63,60.27 -x- Frostwall/0 57.18,06.60 {template:custom} {atob:Enter Garrison} {btoa:Leave Garrison}", -- 941 is initial Frostfire map
		...
		"Frostfire Ridge/0 49.68,60.39 -x- Frostwall/0 57.26,07.74 {template:custom} {atob:Enter Garrison} {btoa:Leave Garrison}",

		"Shadowmoon Valley D/0 27.2,15.0 -x- Lunarfall/0 16.0,30.0 {template:custom} {atob:Enter Garrison} {btoa:Leave Garrison} {cond:C_Garrison:GetGarrisonInfo()==1}",
		"Shadowmoon Valley D/0 32.4,23.5 -x- Lunarfall/0 63.3,91.3 {template:custom} {atob:Enter Garrison} {btoa:Leave Garrison}",
	}
}
...
-- == PORTALS == --
	...
	-- PANDARIA
		...
		"Stormwind City 25,1 <title:The Skyfire Airship> -to- The Jade Forest,46.23,85.17   (A:PORTAL) {cond:UnitLevel('player')>=85 and not PlayerCompletedQuest(29548)}", -- airship for the initial quest
		"Stormwind City 25,1 <title:The Skyfire Airship> <override_text:You're too low level to enter Pandaria.> <override_icon:error> -to- The Jade Forest,46.23,85.17   (A:PORTAL) {cond:UnitLevel('player')<85}", -- airship for the initial quest
	...
	-- GARRISON PORTALS
		"Frostwall 75.1,48.7 -to- Warspear 44.4,35.5 (H:PORTAL) {cond:C_Garrison:GetGarrisonInfo()==3}", -- Horde Garrison 3 to Ashran
		"Lunarfall 70.2,27.5 -to- Stormshield 31.7,52.5 (A:PORTAL) {cond:C_Garrison:GetGarrisonInfo()==3}", -- Ally Garrison 3 to Ashran
...
data.basenodes.inns = {
	...
	['Stormshield']={
		{name="Hero's Rest Inn",faction="A",npc="Jaesia Rosecheer",npcid=85956,x=35.7,y=77.9},
	},
	['Warspear']={
		{name="Brokenspear Tavern",faction="H",npc="Narnin Dawnglow",npcid=86307,x=45.0,y=43.2},
	},
	['Frostfire Ridge']={
