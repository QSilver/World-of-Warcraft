...
_Heroic Difficulty_: _Rotall_ gains _Scorching Aura_, reducing cast speed when in close proximity by 100%. |only if heroic_dung()
...
_Heroic Difficulty_: _Gug'rokk_ gains _Flame Buffet_, dealing damage to all players and increasing fire damage taken. Stacks. |only if heroic_dung()
...
_Heroic Difficulty_: _Ravenous Wolves_ gain a damage buff when near each other. Keep them separated. |only if heroic_dung()
...
_Heroic Difficulty_: He will now cast _Fate_, instantly blowing up all _Hallowed Ground_ patches immediately. |only if heroic_dung()
...
_Heroic Difficulty_: _Fel Sparks_ will reach out of _Fel Pools_ inflicting extreme fire damage each second to anyone who stands in them. |only if heroic_dung()
...
_Heroic Difficulty_: Run away when the _Felborne Abyssal_ fixates on you. |only if heroic_dung()
_Phase 2:_
...
_Heroic Difficulty_: _Lens Flare_ will call a beam of light down on a party member and follow them. Move away and don't lead it through the group. |only if heroic_dung()
...
_Heroic Difficulty_: _Rukhran_ will cast _Quills_, dealing damage to all enemies and increasing in damage each cast. Hide behind the pillar to avoid damage. |only if heroic_dung()
...
_Heroic Difficulty_: _Borka's Slam_ also interrupts for 1.5 seconds. |only if heroic_dung()
...
_Heroic Difficulty_: Move out of _Slag Blast_. This leaves fire on the ground. |only if heroic_dung()
...
_Heroic Difficulty_: Random players will be marked with _Hunter's Mark_. Move away from them. |only if heroic_dung()
...
_Heroic Difficulty_: _Witherbark_ will continue to cast _Unchecked Growth_ during _Brittle Bark_. |only if heroic_dung()
...
_Heroic Difficulty_: Interrupting _Parasitic Growth_ spawns a _Spore Image_. This image continues casting the prevous school of magic until killed. |only if heroic_dung()
...
_Heroic Difficulty_: It is important to stay away from others when affected by _Salve of Toxic Fumes_. |only if heroic_dung()
...
_Heroic Difficulty_: _Zaela_ gains _Destructive Smite_, dealing massive damage to the current target and knocking them back. |only if heroic_dung()
