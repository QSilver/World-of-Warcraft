...
function Pointer:Startup()
	...
	if ZGV.DEV then self.OverlayFrame.ZygorCoordsDEV = ZGV.ChainCall(self.OverlayFrame:CreateFontString(nil,"ARTWORK","GameFontHighlight")) :SetPoint("BOTTOMLEFT") :SetWidth(200) :SetJustifyH("LEFT") .__END  end
	...
	if ZGV.db.profile.debug then
		...
		:SetPoint("TOPLEFT",WorldMapButton,"TOPLEFT",0,-20)
		:SetSize(100,50)
...
Pointer.Icons = {
	...
	ant =		   { icon=ZGV.DIR.."\\Skins\\minimaparrow-ant", alpha=0.8, size=30, minisize=20, rotates=false, edgeicon=nil, edgesize=1 },
	ant_g = { icon=ZGV.DIR.."\\Skins\\minimaparrow-ant", r=0.4, g=1, b=0, alpha=0.8, size=30, minisize=20, rotates=false, edgeicon=nil, edgesize=1 },
	ant_b =   { icon=ZGV.DIR.."\\Skins\\minimaparrow-ant", r=0, g=0.7, b=1, alpha=0.8, size=30, minisize=20, rotates=false, edgeicon=nil, edgesize=1 },
	ant_p =   { icon=ZGV.DIR.."\\Skins\\minimaparrow-ant", r=0.8, g=0.3, b=1, alpha=0.8, size=30, minisize=20, rotates=false, edgeicon=nil, edgesize=1 },
	ant_y =   { icon=ZGV.DIR.."\\Skins\\minimaparrow-ant", r=1, g=0.8, b=0, alpha=0.8, size=30, minisize=20, rotates=false, edgeicon=nil, edgesize=1 },
...
function Pointer:MakeMarkerFrames(marker,type)
	...
	--print(type=="Ant" and ("MEDIUM" and WorldMapFrame:GetFrameStrata() ~= "HIGH") or "HIGH")
	
	local strataLayer = nil
	if type == "Ant" and WorldMapFrame:GetFrameStrata() ~= "HIGH" then
		strataLayer = "MEDIUM"
	elseif type == "Ant" then
		strataLayer = "HIGH"
	end
	marker.worldmapFrame:SetFrameStrata(strataLayer or "HIGH")
	...
	if Nx then -- Attempt to support Carbonite map, doesn't work.
		...
		if win:IsSizeMax() then
			marker.worldmapFrame:SetParent(win.Frm)
			Spoo(nil, nil, win.Frm)
			--print("Y")
		else
			marker.worldmapFrame:SetParent(win.Frm)
			--marker.worldmapFrame:SetParent(self.OverlayFrame)
			--print("N")
		end
...
function Pointer.MinimapButton_OnClick(self,button)
	if button=="RightButton" then
		...
		elseif self.waypoint.type=="route" then
			...
			if ZGV.db.profile.debug and self.waypoint.pathnode and ZGV.Pointer.ArrowFrame.waypoint.pathnode then
		...
		if ZGV.db.profile.pathfinding then ZGV.Pointer:FindTravelPath(self.waypoint) else ZGV.Pointer:ShowArrow(self.waypoint) end
	end
end
...
function Pointer.ArrowFrame_OnUpdate_Common(self,elapsed)
	...
	then
		...
		then
			if GetTime()<pathfindlockout then
				...
				self:Debug("Pointing to next from OnUpdate_Common arrival")
				...
				if nextway~=waypoint then
					if self.pointsets['route'] then LibRover:Abort() self:ClearSet("route") end  -- UGLY to call it here, but Travel needs to GTFO when next-in-path waypoints reign.
	...
	elseif ArrowFrame.WaitingPhase then
		...
		elseif waypoint.actionicon then
			...
			if ZGV.db.profile.arrowsmooth and self.CurrentArrowSkin and self.CurrentArrowSkin.features.smooth then
	...
	if eta_elapsed >= 0.2 then
		...
		if pathfollow=="smart" or pathfollow=="loose" or pathfollow=="smart2" then
			self:Debug("Pointing to next from OnUpdate_Common timer")
			local newway = self:PointToNextInPath(waypoint.in_set)
			if newway~=waypoint and self.pointsets['route'] then LibRover:Abort() self:ClearSet("route") end
		end
	...
	if LibRover.initializing and not LibRover.ready then
		ArrowFrame:ShowText("Travel System is initializing...",nil,nil,idle_dots[LibRover.initframes%5+1])
		return
	end
...
function Pointer:DoCorpseCheck(event)
	if self:IsCorpseArrowNeeded() then
		self:Debug("DoCorpseCheck (%s) (dead)",event or "no event")
		-- corpse arrow
		if not self.corpsearrow then self:FindCorpseArrow() end
	else
		self:Debug("DoCorpseCheck (%s) (not dead)",event or "no event")
...
local function RestartCorpseSearch(where) -- made it a func not to duplicate code
	...
	ZGV:ScheduleTimer(function() ZGV.Pointer:FindCorpseArrow(true) end,1.0)
end
...
function Pointer:FindCorpseArrow(reset)
	...
	if sysx>0 then
		...
		for i,id in ipairs(submap_cache[system]) do repeat -- Also each map includes self in the list
			...
			Pointer:Debug("Corpse check on map %d (%s), levels %d-%d",id,(GetMapNameByID(id) or "?"),(levels>0 and 1 or 0),levels)
...
function Pointer:PointToNextInPath(pathname)
	...
	if type(way)=="table" then  -- OK, waypoint
		...
		self:ShowArrow(way)
		if LibRover.updating then LibRover:Abort() end  -- BAD PLACE :(
		return way
	else
		self:Debug("PointToNext %s: to %s ?",pathname,tostring(way))
	end
end
...
function Pointer:GetNextInPath(pathname,testway)
	...
	if pathfollow == "smart" then
		...
		local smart_path_dist = pointset.smarttight or 50
		...
		for w,way in ipairs(pathpoints) do  if way.minimapFrame then
			...
			if dist_between>smart_path_dist*0.5 then
				...
				if dist<smart_path_dist then pull=0 end  -- zero pull at start point of each segment!
	...
	elseif pathfollow == "smart2" then ----------------------------------------
		...
		local nears={}
		local nearnum
		for w,way in ipairs(pathpoints) do  if way.minimapFrame and way.minimapFrame.dist<neardist then  neardist=way.minimapFrame.dist  nearnum=w  end  end
		tinsert(nears,nearnum)
		repeat
			local neardistmin=neardist
			neardist=neardist*1.1
			for w,way in ipairs(pathpoints) do  if way.minimapFrame and way.minimapFrame.dist>=neardistmin and way.minimapFrame.dist<neardist then  tinsert(nears,w)  end  end
	...
	elseif pathfollow == "strict"  then ----------------------------------------
		...
		for w,way in ipairs(pathpoints) do
			...
			and way.minimapFrame and way.minimapFrame.dist<(way.radius or default_radius)  -- and we're standing on top of it
			then  -- skip to the next.
				stopatnextway=true
			end
		end
		if stopatnextway and pointset.loop then return pathpoints[1] else return curway end

	elseif pathfollow == "loose" then ----------------------------------------

		if not curway then -- default to closest
			local nearest_dist,nearest_way=9999,nil
			for w,way in ipairs(pathpoints) do if way.minimapFrame and way.minimapFrame.dist<nearest_dist then nearest_dist,nearest_way=way.minimapFrame.dist,way end end
			return nearest_way
		end
		...
		if stopatnextway and pointset.loop then return pathpoints[1] end
		if curway then return curway end
...
function Pointer:TestGetNextInPathOne(x,y)
	...
	way.worldmapFrame.icon:SetVertexColor(unpack(colors[nextway] or defaultcolor))
end
...
local function set_waypoints(points,worldsize,minisize,ptype,setname)
	...
	for k,point in ipairs(points) do
		if not point.force_noway and point.x then
			...
			for k in pairs(copy_fields) do if point[k]~=nil then data[k]=point[k] end end
...
function Pointer:SetAntSpacing(spacing)
	...
	--self:Debug("ant spacing %.1f",spacing)
end
...
local function PathFoundHandler(state,path,ext)
	...
	Pointer:Debug("&_PUSH PathFoundHandler! state=%s",state)
	...
	if state=="success" then
		...
		for i,node in ipairs(path) do
			...
			elseif mode=="ship" or mode=="zeppelin" then wayp.ant_icon = ZGV.Pointer.Icons.ant_b
			elseif mode=="portal" or mode=="teleport" or mode=="hearth" or mode=="ghearth" or mode=="astralrecall" or mode=="courtesy" or mode=="useitem" then wayp.ant_icon = ZGV.Pointer.Icons.ant_p
	...
	Pointer:Debug("&_POP PathFoundHandler done.")
end
...
function Pointer:FindTravelPath(way)
	...
	if type(way)=="table" then
		...
		Pointer:Debug("FindTravelPath to %s",waypoint_tostring(way))
	...
	elseif type(way)=="string" then
		...
		if not set then Pointer:Debug("No set '%s'",setname) return end
		if not set[1] then Pointer:Debug("No points in set '%s'",setname) return end
		way=set[1]
		...
		Pointer:Debug("FindTravelPath to pointset '%s'",setname)
		ZGV.Pointer.DestinationWaypoint = way
		local more_points = {}
		for i=2,#set do
			local w=set[i]
...
function Pointer:CycleWaypoint(delta,nocycle)
	self:Debug("Cycling waypoint, delta "..(delta or "?"))
	...
	repeat
		cycles=cycles+1
		if cycles>50 then return end  --cycling forever? out.
		...
		if CS.current_waypoint_goal_num==old_goal_num then return end --full cycle or no change at all, abort
	...
	if self.waypoints then for wi,way in ipairs(self.waypoints) do
		if way.goal==goal then
			...
			if WorldMapFrame:IsShown() then SetMapByID(way.m) SetDungeonMapLevel(way.f or 1) end
			break
		end
	end
	else
	...
	--ZGV:SetWaypoint("goal",CS.goals[CS.current_waypoint_goal_num])
	
	--CS.goals[CS.current_waypoint_goal_num]
	--ZGV:SetWaypoint(CS.current_waypoint_goal_num)
...
function Pointer:CycleWaypointTo(goalnum)
	...
	Pointer:CycleWaypoint(1,1) -- Force waypoint to cycle to first one.
end
