...
function ClassMods.CheckForDebuff(spell, target, owner)
	local name, rank, icon, count, _, duration, expirationTime, _, _, _, spellId = UnitAura(target, tonumber(spell) and GetSpellInfo(tonumber(spell) ) or spell, nil, (owner == "PLAYERS") and "PLAYER|HARMFUL" or "HARMFUL")
...
function ClassMods.CheckForBuff(spell, target, owner)
	local name, _, icon, count, debuffType, duration, expirationTime = UnitAura(target, tonumber(spell) and GetSpellInfo(tonumber(spell) ) or spell, nil, (owner == "PLAYERS") and "PLAYER|HELPFUL" or "HELPFUL")
	return (name and icon or nil), (name and duration or 0), ( (name and (expirationTime - GetTime() ) > 0) and math.max(expirationTime - GetTime(), 0) or 0), (count)
end
