...
--	if not ZygorGuidesViewerMiniFrame then error("Zygor Guide Viewer step frame not loaded.") end
	...
	if addonName:find("BETA") then self.DEV=true ZygorGuidesViewerFrame_DevLabel:SetText("BETA") end
...
function ZGV:Startup_LoadGuides_Threaded()
	...
	if #self.registeredguides>0 then
		...
		for i,guide in ipairs(self.registeredguides) do repeat
			...
			group = group and self:FindGroup(self.registered_groups,group) or self.registered_groups
			guide.ord=#group.guides+1
			tinsert(group.guides,guide)


			if debugprofilestop()-t>300 then  -- let it take us down to 3fps, it's the startup.
				self.loadprogress = i/#self.registeredguides
				self:SendMessage("ZYGORGV_LOADING")
				t=debugprofilestop()
				yield(full_load and "parsing fully" or "parsing headers")
			end
		until true end
...
local function _StartupThread()
	...
	for i,startup in ipairs(self.startups) do
		...
		repeat
			...
			if self.db.profile.safe_startup then t=debugprofilestop() yield(("Startup module: %s took %d ms"):format(name,t1)) end

			if not self.db.profile.safe_startup then self:Debug("&startup Startup module: %s in %d ms",name,t1) end
			if not ok then  self:Error(ret)  end
...
function ZGV:StartupStep()  -- called from MasterFrame
	...
	if ZGV:IsPlayerInCombat() then
		if not displayedInCombatWarning then
			...
			self:Print("You are in combat! Pausing guide startup.")
			displayedInCombatWarning = true
		end
		return
	end -- wait for combat to end, since it breaks arrow
...
function ZGV:OnFirstQuestLogUpdate()
	...
	if ZGV.db.char.maint_startup_startguide then
		...
		if self.db.char.guidename and self.db.char.guidename=="GOLD\\Crafting\\Dynamic Crafting Guide" then
...
function ZGV:FindSuggestedGuides()
	local suggested={}
	for i,guide in ipairs(self.registeredguides) do
		if guide:GetStatus()=="SUGGESTED" or guide.sugGroup then
...
function ZGV:FocusStepUnquiet()
	...
	for _,sticky in ipairs(stickies) do
		if not sticky:IsComplete() or self.CurrentStep:IsComplete() then self:SetWaypoint("sticky",sticky) end
	end
end
...
function ZGV:SkipStep(fast,hack) --Hack used for testing.
	if not self.CurrentGuide then return end
		...
		if self.CurrentGuide and self.CurrentGuide.steps and self.CurrentGuide.fully_parsed then
			...
			-- FIRST, assign steps to frames.
				...
				-- automate adding steps to stepframes. Refuse to add completed stickies, or wrong steps.
					local framenum=1
					local function AddStep(step,sticky)
						if framenum>self.StepLimit then return end
						...
						self.stepframes[framenum].step = step
						self.stepframes[framenum].stepnum = step.num
						self.stepframes[framenum].is_sticky = not not sticky
						framenum=framenum+1
					end

				-- "First" step goes, well, first.
					AddStep(self.CurrentGuide.steps[firststep])
				
				-- Now the stickies that accompany it, if any.
					if stickies then
						for _,sticky in ipairs(stickies) do
							if not iscomplete then  AddStep(sticky,"sticky")  end
						end
					end

				-- And now any future steps, if any.
					for stepnum=firststep+1,laststep do  AddStep(self.CurrentGuide.steps[stepnum])  end

				-- Clear out steps from further frames.
					for f=framenum,self.StepLimit do self.stepframes[f].step=nil self.stepframes[f].stepnum=nil end
			...
			for stepframenum = 1,self.StepLimit do repeat
				...
				if showallsteps then
					frame:SetAlpha((stepdata.num<self.CurrentStepNum and 0.3) or (stepdata.num==self.CurrentStepNum and 1.0) or 0.8)
				else
...
function ZGV:Debug (msg,...)
	...
	if self and self.db and self.db.profile and self.db.profile.debug and not self.db.profile.quiet then
		self.DebugI = (self.DebugI or 0) + 1
		--func = func:match("in function `(.-)'") or func
		--func = func:match("in function.-string \"*(:.-)\"") or func
		local t = GetTime()
		if t~=last_t then
			framestart_t = debugprofilestop()
			mscycle=not mscycle
			last_t=t
			timecolor=mscolors[mscycle and 1 or 2]
		end
		local debugms = debugprofilestop()-framestart_t
